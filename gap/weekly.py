...
def build_week_report(start: str | None = None, end: str | None = None) -> tuple[str, str]:
    ...
    lines.append("paper_full_at_limit = 100% of intended size at the limit.")
    lines.append("No shared liquidity across A/B/C/D. No bankroll cap. No cluster cap.")
    lines.append("Scalp books C/D have no minute tape in paper, so weekly marks them")
    lines.append("at official settlement and tags scalp_unobserved_settled_as_hold.")
    lines.append("Do not treat C/D weekly PnL as a real scalp until a tape exists.")
    lines.append("")
